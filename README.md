...
방명록 작성자는 글을 남길 때 `수정/삭제 비밀번호`를 함께 입력합니다. 이후 메인 페이지의 각 방명록에서 `수정/삭제`를 눌러 같은 비밀번호를 입력하면 본인 글을 수정하거나 삭제할 수 있습니다.

기존에 이미 작성된 방명록은 작성 비밀번호가 없으므로 일반 사용자가 직접 수정/삭제할 수 없습니다. 관리자는 `/admin.html`에서 수정/삭제할 수 있습니다.
